...
Reader.prototype.mostRecentWindow = function mostRecentWindow(from, to, next) {
  ...
  // Find some documents
  this.collection
    .find({ pair: this.pair, start: { $gte: mFrom, $lte: mTo } })
    .sort({ start: -1 }, (err, docs) => {
      if (err) {
      ...
      var gapIndex = _.findIndex(
        docs,
        (r, i) => r.start !== mostRecent - i * 60
      );
...
Reader.prototype.get = function get(from, to, what, next) {
  // returns all fields in general
  // Find some documents
  this.collection
    .find({ pair: this.pair, start: { $gte: from, $lte: to } })
    .sort({ start: 1 }, (err, docs) => {
      if (err) {
...
Reader.prototype.count = function fCount(from, to, next) {
  this.collection.count(
    { pair: this.pair, start: { $gte: from, $lte: to } },
    (err, count) => {
      if (err) {
